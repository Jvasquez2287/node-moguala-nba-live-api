...
class GroqRateLimiter {
   ...
   */
  constructor(
    maxRequestsPerMinute: number = 28,
    maxTokensPerMinute: number = 5800,
    tokensPerRequest: number = 1000
  ) {
  ...
  async waitIfNeeded(estimatedTokens?: number): Promise<void> {
    ...
    try {
      ...
      if (waitTime > 0) {
        console.log(
          `[Groq Rate Limit] Waiting ${waitTime.toFixed(1)}s ` +
...
export async function callGroqApi(
  ...
  userPrompt: string,
  rateLimiter: GroqRateLimiter = groqRateLimiter
  ...
  try {
    ...
    if (
      ...
      if (waitTime) {
        console.warn(
          `[Groq] Rate limit exceeded. Waiting ${waitTime.toFixed(1)}s...`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, Math.ceil(waitTime! * 1000))
        );
