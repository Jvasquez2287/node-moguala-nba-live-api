...
export const migrationService = {
  ...
  async getExecutedMigrations(): Promise<any[]> {
    try {
      const result = await executeQuery(
        'SELECT migration FROM migrations ORDER BY batch, executed_at',
        {}
      );
  ...
  async runPendingMigrations(): Promise<void> {
    try {
      ...
      const executedNames = executedMigrations.map((m: any) => m.migration);
      ...
      const pendingMigrations = migrationFiles.filter(
        m => !executedNames.includes(m.filename) && !m.filename.startsWith('000_')
      );
