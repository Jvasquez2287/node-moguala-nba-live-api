...
class DataCache {
    constructor() {
        ...
        this.boxScoreCache = new LRUCache(50); // Box scores for recent games
        // Polling intervals
        this.SCOREBOARD_POLL_INTERVAL = 8000; // 8 seconds (real-time)
        this.PLAYBYPLAY_POLL_INTERVAL = 5000; // 5 seconds (real-time)
    ...
    setTeamRoster(teamId, season, data) {
        const key = `${teamId}_${season}`;
    ...
    async pollScoreboard() {
        ...
        const poll = async () => {
            ...
            try {
                ...
                const response = await axios_1.default.get('https://cdn.nba.com/static/json/liveData/scoreboard/todaysScoreboard_00.json', {
                ...
                const transformedData = {
                    scoreboard: {
                        ...
                        games: (scoreboardData.scoreboard?.games || []).map((game) => ({
                            gameId: game.gameId,
            ...
            try {
                ...
                for (const gameId of gamesToPoll) {
                    ...
                    try {
                        ...
                        const response = await axios_1.default.get(`https://cdn.nba.com/static/json/liveData/playbyplay/playbyplay_${gameId}.json`, {
                            timeout: 30000, // 30 second timeout
                            headers: {
                        ...
                        const transformedData = {
                            ...
                            plays: (playbyplayData.game?.actions || []).map((action) => ({
