...
const playerCache = new Map<string, CacheEntry<PlayerSummary>>();

// Cache for player search results
const playerSearchCache = new Map<string, CacheEntry<PlayerSummary[]>>();
...
export async function getPlayer(playerId: string): Promise<PlayerSummary> {
    try {
       ...
       console.log(`Cache miss for player ${playerId}, fetching from API`);
        ...
        const playerData = playerResponse.resultSets[0].rowSet.find((row: any[]) => row[0] === playerIdNum);
        if (!playerData) {
            throw new Error(`Player not found with ID: ${playerId}`);
        }
        ...
        const recentGames: PlayerGamePerformance[] = [];
        try {
...
export async function searchPlayers(query: string): Promise<PlayerSummary[]> {
    try {
       ...
       console.log(`Cache miss for player search "${query}", fetching from API`);
        ...
        const filteredPlayers = allPlayers.filter((row: any[]) => {
        ...
        const players: PlayerSummary[] = filteredPlayers.map((row: any[]) => {
            const getValue = (headerName: string) => {
                const index = headers.indexOf(headerName);
 ...
 */
export async function getSeasonLeaders(season: string = "2024-25"): Promise<SeasonLeadersResponse> {
    try {
        // Check cache first
        const seasonParts = season.split('-');
        const nbaSeasonFormat = `${seasonParts[1]}/${seasonParts[2]}/${seasonParts[0]}`;

        const cachedData = await dataCache.getSeasonLeaders(season);
       ...
       console.log(`Cache miss for season leaders ${season}, fetching from API`);
        ...
        const createLeaders = (statCol: string, categoryName: string, topN: number = 5, isPercentage: boolean = false, isWholeNumber: boolean = false) => {
            ...
            const leaders = filteredPlayers.map((row: any[]) => {
        ...
        // Cache the result
        dataCache.setSeasonLeaders(season, result);
 ...
 */
export async function getLeagueRoster(): Promise<PlayerSummary[]> {
    try {
        // Check cache first
        const cachedData = await dataCache.getLeagueRoster();
        if (cachedData) {
       ...
       console.log(`Cache miss for league roster, fetching from API`);
        ...
        const activePlayers = allPlayers.filter((row: any[]) => {
            const rosterStatus = row[headers.indexOf('ROSTER_STATUS')];
            return rosterStatus === 1 || rosterStatus === '1';
        });

        const players: PlayerSummary[] = activePlayers.map((row: any[]) => {
            const getValue = (headerName: string) => {
                const index = headers.indexOf(headerName);
                return index !== -1 ? row[index] : null;
            };
        ...
        // Cache the result
        dataCache.setLeagueRoster(players);
