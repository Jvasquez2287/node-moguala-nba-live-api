...
router.get('/:gameId', async (req, res) => {
    try {
        ...
        const moments = await keyMoments_1.keyMomentsService.getKeyMomentsForGame(gameId);
...
router.get('/', async (req, res) => {
    try {
        ...
        const allMoments = [];
        const liveGames = scoreboardData.scoreboard.games.filter((game) => game.gameStatus === 2 // Live games
        );
        for (const game of liveGames) {
            try {
                const moments = await keyMoments_1.keyMomentsService.getKeyMomentsForGame(game.gameId);
