...
router.get('/email-config', (req, res) => {
    try {
        const isReady = emailService_1.emailService.isReady();
        return res.json({
            success: true,
...
router.post('/send-success-email', async (req, res) => {
    try {
        ...
        if (!emailService_1.emailService.isReady()) {
            return res.status(503).json({
                success: false,
                error: 'Email service not configured. Set EMAIL_HOST, EMAIL_USER, EMAIL_PASSWORD in .env'
            });
        }
        const success = await emailService_1.emailService.sendSuccessEmail({
            userEmail: email,
            userName: name,
...
router.post('/send-error-email', async (req, res) => {
    try {
        ...
        const success = await emailService_1.emailService.sendErrorEmail({
            userEmail: email,
            userName: name,
            errorMessage,
        });
...
router.post('/send-renewal-email', async (req, res) => {
    try {
        ...
        const success = await emailService_1.emailService.sendRenewalEmail({
            userEmail: email,
            userName: name,
            subscriptionStatus: 'ACTIVE',
