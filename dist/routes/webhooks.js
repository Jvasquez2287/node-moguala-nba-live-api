    ...
    try {
        ...
        switch (event.type) {
            case 'customer.subscription.created': {
                ...
                const customerEmail = await stripe_1.stripeService.getEmailByCustomerId(data.customer) || subscriptionData.stripe_id;
                // Send success email
                try {
                    if (customerEmail) {
                        ...
                        await emailService_1.emailService.sendSubscribedEmail({
            ...
            case 'customer.subscription.updated': {
                ...
                const previousSub = await stripe_1.stripeService.getSubscriptionFromDBWithSubscriptionId(subscriptionData.subscription_id);
                ...
                try {
                    if (customerEmail) {
                        ...
                        if (data.status === 'active' && previousStatus !== 'active') {
                            ...
                            await emailService_1.emailService.sendReactivateEmail({
                                userEmail: customerEmail,
                                subscriptionTitle: productData.name,
                                periodStart,
                                periodEnd,
            ...
            case 'customer.subscription.deleted': {
                ...
                try {
                    ...
                    if (customerEmail) {
                        ...
                        try {
                            const product = await stripe_1.stripeService.getProductsByID(data.items.data[0].plan.product);
