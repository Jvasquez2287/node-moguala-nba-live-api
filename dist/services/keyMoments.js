...
exports.keyMomentsService = void 0;
exports.startCleanupTask = startCleanupTask;
exports.startProcessingTask = startProcessingTask;
exports.stopCleanupTask = stopCleanupTask;
exports.stopProcessingTask = stopProcessingTask;
...
(function (KeyMomentType) {
    KeyMomentType["GAME_TYING_SHOT"] = "game_tying_shot";
    KeyMomentType["LEAD_CHANGE"] = "lead_change";
    KeyMomentType["SCORING_RUN"] = "scoring_run";
    KeyMomentType["CLUTCH_PLAY"] = "clutch_play";
    KeyMomentType["BIG_SHOT"] = "big_shot";
})(KeyMomentType || (KeyMomentType = {}));
// Cache for recent key moments per game (stores last 5 minutes of moments)
const keyMomentsCache = new Map();
const lastCheckedPlays = new Map(); // game_id -> last action_number checked
const momentContextCache = new Map();
 ...
 */
function getMomentContext(momentId) {
 ...
 */
function setMomentContext(momentId, context) {
...
async function cleanupFinishedGames() {
    try {
        ...
        if (removedMoments > 0 || removedContexts > 0) {
            console.log(`[KeyMoments] Cleaned up finished games: ${finishedGameIds.length} games, ` +
 ...
 */
function detectGameTyingShot(play, previousHomeScore, previousAwayScore, currentHomeScore, currentAwayScore) {
 ...
 */
function detectLeadChange(previousHomeScore, previousAwayScore, currentHomeScore, currentAwayScore) {
 ...
 */
function detectScoringRun(recentPlays, teamTricode, period) {
 ...
 */
function detectClutchPlay(play, period, clock, homeScore, awayScore) {
 ...
 */
async function detectKeyMoments(gameId) {
    try {
        ...
        for (let i = 0; i < newPlays.length; i++) {
            ...
            if (i < newPlays.length - 1) {
                ...
                const prevPlay = plays.find((p) => p.action_number === prevAction);
                if (prevPlay) {
                    previousHomeScore = parseScore(prevPlay.score_home) || playHomeScore;
                    previousAwayScore = parseScore(prevPlay.score_away) || playAwayScore;
                }
            ...
            if (detectGameTyingShot(play, previousHomeScore, previousAwayScore, playHomeScore, playAwayScore)) {
            ...
            if (detectLeadChange(previousHomeScore, previousAwayScore, playHomeScore, playAwayScore)) {
                detectedMoments.push({
            ...
            if (detectClutchPlay(play, playPeriod, playClock, playHomeScore, playAwayScore)) {
                detectedMoments.push({
            ...
            if (detectBigShot(play, previousHomeScore, previousAwayScore, playHomeScore, playAwayScore)) {
                detectedMoments.push({
 ...
 */
async function getKeyMomentsForGame(gameId) {
    try {
        // Get cached moments
 ...
 */
async function generateBatchedMomentContexts(momentsWithGameInfo) {
    ...
    if (momentsNeedingContext.length === 0) {
        console.log(`[KeyMoments] Returning ${Object.keys(cachedContexts).length} cached contexts - skipping Groq calls`);
        return cachedContexts;
    }
    if (Object.keys(cachedContexts).length > 0) {
        console.log(`[KeyMoments] Found ${Object.keys(cachedContexts).length} cached contexts, ` +
 ...
 */
async function processLiveGames() {
    try {
        ...
        const allMomentsNeedingContext = [];
            ...
            try {
                ...
                if (moments.length > 0) {
                    ...
                    if (game) {
                        ...
                        await websocketManager_1.webSocketManager.broadcastKeyMomentsToAllClientsScoreBoard({
                            type: 'key_Moments',
                            moments,
                            gameId
                        });
        ...
        if (allMomentsNeedingContext.length > 0) {
            console.log(`[KeyMoments] Generating batched context for ${allMomentsNeedingContext.length} moments`);
...
exports.default = exports.keyMomentsService;
