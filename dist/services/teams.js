...
async function getTeam(teamId) {
    try {
        // Check cache first
        const cachedData = await dataCache_1.dataCache.getTeam(teamId);
        if (cachedData) {
        ...
        const apiResponse = await retryAxiosRequest(async () => {
            const response = await axios_1.default.get('https://stats.nba.com/stats/leaguestandingsv3', {
                headers: {
                    "Host": "stats.nba.com",
        ...
        const teamData = teamsData.find((row) => getValue(row, 'TeamID') === teamId);
        if (!teamData) {
        ...
        // Cache the result
        dataCache_1.dataCache.setTeam(teamId, teamDetails);
        return teamDetails;
    }
    catch (error) {
...
async function getAllTeams() {
    try {
        // Check cache first
        const cachedData = await dataCache_1.dataCache.getAllTeams();
        if (cachedData) {
        ...
        const teams = teamsDataRows.map((teamData) => ({
        ...
        // Cache the result
        dataCache_1.dataCache.setAllTeams(teams);
        return teams;
    }
    catch (error) {
        // Enhanced error handling with specific error types
 ...
 */
async function getTeamPlayerStats(teamId, season) {
    try {
        const seasonParam = season.includes('-') ? season : `${season}-${(parseInt(season) + 1).toString().slice(-2)}`;
        ...
        const rosterResponse = await retryAxiosRequest(async () => {
            return await axios_1.default.get('https://stats.nba.com/stats/commonteamroster', {
                headers: {
                    "Host": "stats.nba.com",
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
                    "Accept": "application/json, text/plain, */*",
        ...
        try {
            const statsResponse = await retryAxiosRequest(async () => {
                return await axios_1.default.get('https://stats.nba.com/stats/playercareerstats', {
                    headers: {
                        "Host": "stats.nba.com",
                        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
                        "Accept": "application/json, text/plain, */*",
            ...
            if (statsResponse.data?.resultSets?.[0]) {
                ...
                const getStatsValue = (row, headerName) => {
                    const index = statsHeaders.indexOf(headerName);
                    return index !== -1 ? row[index] : null;
                };
                // Match and update player stats
                for (let i = 0; i < players.length; i++) {
                    const playerStat = statsData.find((stat) => getStatsValue(stat, 'PLAYER_ID') === players[i].player_id);
                    if (playerStat) {
