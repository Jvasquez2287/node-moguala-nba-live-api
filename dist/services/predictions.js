...
const groqPrompts_1 = require("./groqPrompts");
 ...
 */
function isHistoricalDate(dateString) {
...
function fixUnterminatedStrings(jsonStr) {
    ...
    if (openBrackets > 0)
        finalResult += ']'.repeat(openBrackets);
    if (openBraces > 0)
        finalResult += '}'.repeat(openBraces);
 ...
 */
function generateSimpleInsights(homeTeamName, awayTeamName, homeWinProb, predictedHomeScore, predictedAwayScore, homeWinPct, awayWinPct, homeNetRating, awayNetRating) {
    const insights = [];
 ...
 */
async function generateAIInsights(homeTeamName, awayTeamName, homeWinProb, predictedHomeScore, predictedAwayScore, homeWinPct, awayWinPct, homeNetRating, awayNetRating) {
    try {
        const groqApiKey = process.env.GROQ_API_KEY;
        if (!groqApiKey) {
            console.log('[Predictions] Groq API key not configured, using simple insights');
            return generateSimpleInsights(homeTeamName, awayTeamName, homeWinProb, predictedHomeScore, predictedAwayScore, homeWinPct, awayWinPct, homeNetRating, awayNetRating);
        }
        const homeWinProbPct = homeWinProb * 100;
        const awayWinProbPct = (1.0 - homeWinProb) * 100;
        ...
        const prompt = (0, groqPrompts_1.buildInsightPrompt)({
            homeTeamName,
            awayTeamName,
            homeWinProbPct,
            awayWinProbPct,
        ...
        const systemMessage = (0, groqPrompts_1.getSystemMessage)();
        const response = await (0, groqClient_1.callGroqApi)(groqApiKey, systemMessage, prompt);
        ...
        return insights.length > 0 ? insights : generateSimpleInsights(homeTeamName, awayTeamName, homeWinProb, predictedHomeScore, predictedAwayScore, homeWinPct, awayWinPct, homeNetRating, awayNetRating);
    }
    catch (error) {
        console.warn('[Predictions] Error generating AI insights, using simple insights:', error);
        return generateSimpleInsights(homeTeamName, awayTeamName, homeWinProb, predictedHomeScore ?? 0, predictedAwayScore ?? 0, homeWinPct, awayWinPct, homeNetRating, awayNetRating);
    }
}
/**
 * Generate batched AI insights for multiple games in one Groq call
 */
async function generateBatchedAIInsights(predictions) {
    ...
    try {
        ...
        const prompt = (0, groqPrompts_1.buildBatchedInsightsPrompt)(formattedData);
        const systemMessage = (0, groqPrompts_1.getBatchedInsightsSystemMessage)();
        const response = await (0, groqClient_1.callGroqApi)(groqApiKey, systemMessage, prompt);
        let content = response.content;
        // Extract JSON from markdown code blocks 
        if (content.includes('```json')) {
        ...
        try {
            ...
            console.error('[Predictions] JSON parse error after fixMissingCommas:', parseError.message);
 ...
 */
async function generateEnhancedAnalysis(predictions) {
    if (predictions.length === 0)
        return {};
    try {
        const groqApiKey = process.env.GROQ_API_KEY;
        ...
        const gamesText = predictions.map(p => `Game: ${p.awayTeamName} @ ${p.homeTeamName}\n` +
            `Predicted Score: ${p.predictedHomeScore}-${p.predictedAwayScore}\n` +
            `Win Probability: ${p.homeTeamName} ${(p.homeWinProb * 100).toFixed(1)}%`).join('\n\n');
...
async function predictGamesForDate(date, season) {
    try {
        ...
        const dbCached = await dataCache_1.dataCache.getPredictionsForDate(date);
        if (dbCached) {
        ...
        for (const [gameId, gameData] of gameDataMap) {
            try {
                ...
                const insights = batchedInsights[gameId] || generateSimpleInsights(gameData.homeTeamName, gameData.awayTeamName, gameData.homeWinProb, gameData.predictedHomeScore, gameData.predictedAwayScore, gameData.homeWinPct, gameData.awayWinPct, gameData.homeNetRating, gameData.awayNetRating);
                // Determine confidence tier
                let confidenceTier = 'medium';
                if (gameData.confidence >= 0.7)
                    confidenceTier = 'high';
                else if (gameData.confidence >= 0.5)
        ...
        if (isHistoricalDate(date)) {
            ...
            dataCache_1.dataCache.setPredictionsForDate(date, result);
