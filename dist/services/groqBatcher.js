...
exports.generateBatchedGroqResponses = generateBatchedGroqResponses;
exports.clearBatchCache = clearBatchCache;
 ...
 */
async function generateBatchedGroqResponses(options) {
    const { items, buildPromptFn, getSystemMessageFn, parseResponseFn, cacheKeyFn, cacheTtl = 60.0, timeout = 10.0, emptyResult = {} } = options;
    ...
    if (cacheKeyFn) {
        ...
        if (cached && Date.now() / 1000 - cached.timestamp < cacheTtl) {
            console.log(`[Groq Batcher] Returning cached response for ${items.length} items`);
            return cached.data;
        }
        else if (cached) {
    ...
    try {
        ...
        try {
            ...
            response = await (0, groqClient_1.callGroqApi)(process.env.GROQ_API_KEY || '', systemMessage, prompt, (0, groqClient_1.getGroqRateLimiter)());
        ...
        if (cacheKey && Object.keys(results).length > 0) {
            ...
            console.log(`[Groq Batcher] Cached response for ${items.length} items`);
        }
        console.log(`[Groq Batcher] Generated response for ${items.length} items, ` +
...
function cleanupBatchCache() {
    ...
    if (batchCache.size > BATCH_CACHE_MAX_SIZE) {
        ...
        console.log(`[Groq Batcher] LRU eviction: removed ${keysToRemove.length} old entries`);
    }
    if (expiredKeys.length > 0) {
        console.log(`[Groq Batcher] Cleaned up ${expiredKeys.length} expired entries`);
    }
}
/**
...
//# sourceMappingURL=groqBatcher.js.map