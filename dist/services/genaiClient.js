...
exports.callGenaiApi = callGenaiApi;
exports.getGenaiRateLimiter = getGenaiRateLimiter;
...
class GenAIRateLimiter {
     ...
     */
    constructor(maxRequestsPerMinute = 20, maxTokensPerMinute = 5500, tokensPerRequest = 2000) {
        this.requestHistory = [];
        this.tokenHistory = [];
        this.locked = false;
    ...
    async waitIfNeeded(estimatedTokens) {
        ...
        try {
            ...
            // Wait if needed
            if (waitTime > 0) {
                console.log(`[GenAI Rate Limit] Waiting ${waitTime.toFixed(1)}s ` +
 ...
 */
async function callGenaiApi(apiKey, systemMessage, userPrompt, rateLimiter = genaiRateLimiter) {
    ...
    try {
        ...
        if (errorStr.includes('429') ||
            ...
            if (waitTime) {
                console.warn(`[GenAI] Rate limit exceeded. Waiting ${waitTime.toFixed(1)}s...`);
                await new Promise((resolve) => setTimeout(resolve, Math.ceil(waitTime * 1000)));
...
//# sourceMappingURL=genaiClient.js.map